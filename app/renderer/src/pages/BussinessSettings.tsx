...
export const BusinessSettings = ({ onDone }: { onDone?: () => void }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const p = (cfg?.businessProfile ?? {}) as BusinessProfile;
  ...
  const onPickLogo = async (file: File | null) => {
    ...
    // ✅ limita tamaño para que no “reviente” el config
  ...
  return (
    <div className="main">
      <div className="card grid">
        ...
        <label>
          <b>Logo (PNG/JPG)</b>
          <input type="file" accept="image/png,image/jpeg" onChange={(e) => onPickLogo(e.target.files?.[0] ?? null)} />
        </label>

        {logoDataUrl && (
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            <img src={logoDataUrl} alt="logo" style={{ width: 56, height: 56, objectFit: 'contain', borderRadius: 10 }} />
            <button className="btn btn--ghost" onClick={() => setLogoDataUrl('')}>Quitar logo</button>
          </div>
        )}
